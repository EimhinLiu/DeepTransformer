...
train_loader = RandomNodeLoader(data, num_parts=136, shuffle=True, num_workers=0) # batch_size=num_nodes/num_parts
test_loader = RandomNodeLoader(data, num_parts=17, num_workers=0)
...
early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(os.path.dirname(__file__), 'checkpoints', str(conv_layers)+'_'+GNNConv+'_'+skip_connection+'_Checkpoint.pt'))
...
for epoch in range(1, 301):
    ...
    test_variance = utils.np.var(sum((y_pred>0).float().tolist(), []))
    ...
    print(f'TrainLoss: {train_loss:.4f}, ValidLoss: {valid_loss:.4f}, '
          ...
          f'TestVariance: {test_variance:.4f}')
    
    metric = [epoch, train_loss, valid_loss, test_rocauc, test_prauc, test_variance]
